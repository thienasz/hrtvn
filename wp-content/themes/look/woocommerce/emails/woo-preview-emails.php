...
function add_preview_email_links( $settings ) {
	...
	foreach ( $settings as $section ) {
		...
		if ( isset( $section['id'] ) && 'email_recipient_options' == $section['id'] &&
			...
			$updated_settings[] = array(
				'title' => __( 'Preview Email Templates', 'look' ),
				'type'  => 'title',
				'desc'  => '<a href="' . site_url() . '/wp-admin/admin-ajax.php?action=previewemail&file=customer-new-account.php" target="_blank">'.__('Click Here to preview all of your Email Templates with Orders','look').'</a>.',
