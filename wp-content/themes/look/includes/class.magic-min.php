...
class Minifier {
        ...
        if( preg_match_all( '/url\((["\']?)((?!["\']?data:).*?\.(gif|png|jpg|jpeg))\\1\)/i', $contents, $this->matches, PREG_SET_ORDER ) )
        {
            ...
            foreach( $this->matches as $this->graphic )
            {
                ...
                if( $this->remote_file( $this->graphic[2] ) )
                {
                    ...
                    if( file_get_contents($this->graphic[2]))
                    {

                        //Get the image file

                        $this->image_file = file_get_contents($this->graphic[2]);
                        //Get the remote filesize
                        $this->filesize = filesize($this->graphic[2] );
    ...
    public function minify_contents( $src_file, $run_minification = true )
    {
        ...
        if( !$this->source )
        {
            $this->messages[]['Minifier ERROR'] = 'Unable to retrieve the contents of '. $src_file ;
        ...
        {   
            ...
            if( !empty( $this->type ) && $this->type == 'js' )
            {
                ...
                {
                    require_once( THEME_DIR  .'/includes/jShrink.php' );
                }
    ...
    private function make_min( $src_file, $new_file, $do_minify = true )
    {
        ...
        if( $this->settings['gzip'] )
        {
            ...
            $this->prequel .= 'header( \'Last-Modified: ' . gmdate( "D, d M Y H:i:s", filemtime( $src_file ) ) . ' GMT\' );' . PHP_EOL;
